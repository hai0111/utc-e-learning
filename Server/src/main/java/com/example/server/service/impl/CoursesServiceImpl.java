...
public class CoursesServiceImpl implements CoursesService {
    ...
    public ApiResponse<List<CourseResponse>> getPageCourses(int page, int size) {
        ...
        if (FilterRoleUtil.checkRole(authentication).equals(Role.ADMIN) || FilterRoleUtil.checkRole(authentication).equals(Role.INSTRUCTOR)) {
    ...
    @Override
    public ApiResponse<CourseResponse> getCourse(UUID courseId) {
        Authentication authentication = SecurityContextHolder.getContext().getAuthentication();
        UserDetailsImpl userDetails = (UserDetailsImpl) authentication.getPrincipal();
        ...
        if (FilterRoleUtil.checkRole(authentication).equals(Role.ADMIN) || (FilterRoleUtil.checkRole(authentication).equals(Role.INSTRUCTOR))) {
            coursesDto = coursesRepository.findCourseByIdAndRoleAdminAndInstructor(courseId);
            courseResponse = CourseResponse.convertDtoToCourseResponse(coursesDto, 0L);
        } else {
            coursesDto = coursesRepository.findCourseByIdAndStudentId(courseId, userDetails.getId());
            if (coursesDto.getSumProgressAchieved() > 0 || coursesDto.getTotaLessonCount() > 0) {
                progressPercentage = coursesDto.getSumProgressAchieved() / coursesDto.getTotaLessonCount();
            }
            courseResponse = CourseResponse.convertDtoToCourseResponse(coursesDto, progressPercentage);
        }
        return new ApiResponse<>(200, "Success", courseResponse);
    }
    ...
    @Override
    public ApiResponse<List<StudentDto>> getPageStudentsInCourse(int page, int size, UUID courseId, String search) {
        Courses courses = coursesRepository.findByIdAndIsActive(courseId, true);
        if (courses == null) {
            throw new CustomServiceException("This course does not exist", HttpStatus.NOT_FOUND);
        }
        ...
        Pageable pageable = PageRequest.of(page, size);
        Page<StudentDto> result = usersRepository.searchStudentsByCodeOrNameInCourse(courseId, search, pageable);
        return new ApiResponse<>(200, "Success", result.getContent());
    }

    @Override
    public ApiResponse<List<StudentDto>> getPageStudentsNotCourse(int page, int size, UUID courseId, String search) {
        Courses courses = coursesRepository.findByIdAndIsActive(courseId, true);
        if (courses == null) {
            throw new CustomServiceException("This course does not exist", HttpStatus.NOT_FOUND);
        }
        ...
        Pageable pageable = PageRequest.of(page, size);
        Page<StudentDto> result = usersRepository.searchStudentsByCodeOrNameNotCourse(courseId, search, pageable);
        return new ApiResponse<>(200, "Success", result.getContent());
    }
